...
We're getting there, but took the wrong turn. Ghidra would do a much better job at explaining what's going on. So, we just need to compile the program (full instructions can be found [here](https://github.[AWS_SECRET_REMOVED]ADME.md), but it's enough to build an object file):
