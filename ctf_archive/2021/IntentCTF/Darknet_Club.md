...
```
Refused to execute inline script because it violates the following Content Security Policy directive: "default-src 'self'". Either the 'unsafe-inline' keyword, a hash ('sha256-[AWS_SECRET_REMOVED]gL4='), or a nonce ('nonce-...') is required to enable inline execution. Note also that 'script-src' was not explicitly set, so 'default-src' is used as a fallback.
```
...
```console
┌──(user@kali)-[/media/sf_CTFs/intent/Darknet_Club]
└─$ echo [AWS_SECRET_REMOVED]N2gwdTU0bmRfdzByZDV9 | base64 -d
