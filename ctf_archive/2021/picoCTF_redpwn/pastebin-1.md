...
http.createServer((req, res) => {
  ...
  if (!paste) {
    return res.writeHead(200, {
      ...
      'content-security-policy': 'default-src \'none\'; script-src \'self\' \'sha256-[AWS_SECRET_REMOVED]zWw=\''
    }).end(pageNew)
  }
  res.writeHead(200, {
    'content-type': 'text/html',
...
In this case, `script-src` is set to `self` (which means scripts from the same origin are accepted) and `sha256-[AWS_SECRET_REMOVED]zWw=` (which means that scripts that have the given SHA256 signature are accepted). The SHA256 signature is here to allow the following script, which is embedded into the page:

```console
┌──(user@kali)-[/media/sf_CTFs/pico/pastebin-1]
└─$ echo -n "error.innerHTML=decodeURIComponent(location.hash.slice(1))" | sha256sum | awk '{ printf $1 }' | xxd -p -r | base64
[AWS_SECRET_REMOVED]zWw=
```
...
Of course, we, can't use a `<script>` tag since it won't get executed due to the CSP policy. However, using [these CSP bypass guidelines](https://brutelogic.com.br/blog/csp-bypass-guidelines/) we arrive to [this tweet](https://twitter.[AWS_SECRET_REMOVED]3) which contains:
